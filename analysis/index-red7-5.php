...
$WIDTH_CHART 	= 950;
$HEIGHT_CHART	= 50;
$url			= file_get_contents('http://slice.red7.com/get-url-from-hash.php?URL_HASH='.$URL_HASH);

$URL_HASHES		= array(
					'79346372039750b5aafc783a1cf8702f',
					'b3eaf4bce2b85708a6c930a5d527340d'
				);
...
<head>
    ...
    <style>
		...
		.rect_alert {
			fill: #0000ff !important; /* override background */
		}
		.rect_red {
		...
		div.tooltip {   
			position: absolute;           
			text-align: center;           
			width: <?php echo $WIDTH_TT; ?>px;                  
			height: <?php echo $HEIGHT_TT; ?>px;  
			padding: 2px;             
			padding-top:11px;               
...
d3.tsv("<?php echo $URL; ?>", type, function(error, data) {
  y.domain([0, d3.max(data, function(d) { return Math.min(CHART_MAX,d.http_ms); })]);
	...
	div.html("<?php echo $url; ?> { "+data.length+" data points }");
