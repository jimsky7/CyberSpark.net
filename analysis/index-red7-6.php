...
// Write the SVG objects
	...
	foreach ($URL_HASHES as $key=>$URL_HASH) {
		$urls[$key] 			= file_get_contents("http://slice.red7.com/get-url-from-hash.php?URL_HASH=$URL_HASH");
		$getDataURL[$key]	= "/get-log-entries.php?format=tsv&URL_HASH=$URL_HASH&span=$span";
...
<?php
	...
	$stop = count($urls);
	while ($i < $stop) {
		echo  "urlFromHash['".$URL_HASHES[$i]."']='".$urls[$i]."';\r\n";
		$i++;
	}
?>
var getDataURL = [<?php
...
//	div.html("<?php echo $urls[0]; ?> { "+data.length+" data points }");
